...
void copy_union(void)
{
    ...
    fprintf(text_file, "typedef struct");
    if (dflag)
        fprintf(union_file, "typedef struct");
...
{
    int c;
    /* read the keyword, skipping the inital '%j' */
    cptr += 2;
    c = *cptr;
    cinc = 0;
...
int mark_symbol(void)
{
    ...
    if (c == '=')
        cptr += 2;
             ...
             ((c = cptr[2]) == 'r' || c == 'R') && ((c = cptr[3]) == 'e' || c == 'E') && ((c = cptr[4]) == 'c' || c == 'C') && ((c = cptr[5], !IS_IDENT(c))))
        cptr += 5;
    else
        syntax_error(lineno, line, cptr);
